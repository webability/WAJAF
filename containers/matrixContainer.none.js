...
/*
    matrixContainer.js, WAJAF, the WebAbility(r) Javascript Application Framework
    Contains container to control a matrix of zones
...
function matrixZone(maincontainer, domID, domNodeContainer, params, notify)
{
  var self = this;
  this.type = 'zone';
...
function matrixContainer(domNodeFather, domID, params, notify, _4glNode)
{
  var self = this;
  this.type = 'container';
  ...
  this.columns = params.attributes.columns?params.attributes.columns:2;
  ...
  this.templates = {};      // all the knows templates for this matrix
  this.data = {};           // all the data rows we can use for this matrix
  ...
  this.classname = params.attributes.classname!=undefined?params.attributes.classname:'matrix';
  this.classnameline = params.attributes.classnameline!=undefined?params.attributes.classnameline:'matrixline';
  this.classnamezone = params.attributes.classnamezone!=undefined?params.attributes.classnamezone:'matrixzone';
  ...
  function createLine()
  {
    var domNodetr = WA.createDomNode('tr', self.domID+'_tr'+self.trcount, self.classnameline);
    self.domNodetbody.appendChild(domNodetr);
    self.trnodes[self.trcount] = domNodetr;
    for (var i = 0; i < self.columns; i++)
    {
      var domNodetd = WA.createDomNode('td', self.domID+'_td'+self.tdcount, self.classnamezone);
  ...
  function createZone(domID, params, notify)
  {
    ...
    if (pointer === false)
      this.createLine();
    var pointer = self.getFreeTD();
    var z = new matrixZone(this, domID, self.tdnodes[pointer], params, notify);
...
// Needed aliases
WA.Containers.matrixContainer = matrixContainer;
