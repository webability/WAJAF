  ...
  function start(params)
  {
    ...
    if (self.state == 0) // starting
    {
      ...
      else
      {
        ...
        self.params = params;
        var request = WA.Managers.ajax.createRequest(WA.Managers.wa4gl.url + '?P=' + self.applicationID + '.code.json', 'POST', params, getJSON, true);
        return;
      }
    }
...
WA.Managers.wa4gl._container.prototype.listener = function(order, params)
{
  ...
  var request = WA.Managers.ajax.createRequest(WA.Managers.wa4gl.url+'?P='+this._4glNode.application.applicationID + '.' + this._4glNode.id + '.json', 'POST', 'Order='+order, this.responselistener, false);
  if (request)
  {
    for (var i in params)
    {
